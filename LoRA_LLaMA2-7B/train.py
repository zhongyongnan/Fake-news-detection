...
def preprocess(
    ...
    def _tokenize_fn(example):
        ...
        example["input_ids"] = torch.LongTensor(input_id.tolist())
        example["labels"] = torch.LongTensor(label.tolist())
        example["split_ids"] = len(source_input_id)
        return example
...
def format_data(dataset: Dataset, num_proc:int = 1):
    ...
    return dataset.map(add_prompt, remove_columns=["input", "output"], num_proc=num_proc)
...
class SupervisedDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, data_args, tokenizer: transformers.PreTrainedTokenizer):
        ...
        data_path, num_proc= data_args.data_path, data_args.num_proc
        ...
        format_dataset = format_data(dataset, num_proc = num_proc)
